...
Double_t nSigmaTOFKaonTrack_upper(Double_t x) { 
	/*double f_nsigmaTOFKaonTrack_res =  1.34 + (0.02/(pow((x + 0.07), 3.90)));
...
Double_t nSigmaTOFPionTrack_lower(Double_t x) {
    ...
    Double_t nSigmaTOFSoftPionTrack_upper(Double_t x) { 
	double f_nSigmaTOFPionTrack_res =  0.9015 + (0.0001/(pow((x + 0.2003), 9.0378)));
    double f_nSigmaTOFPionTrack_pos = 0.0119 + (0.0139/(pow((x + 0.3327), 6.3391))); 
...
Double_t nSigmaTOFSoftPionTrack_lower(Double_t x) {
	double f_nSigmaTOFPionTrack_res =  0.9015 + (0.0001/(pow((x + 0.2003), 9.0378)));
    double f_nSigmaTOFPionTrack_pos = 0.0119 + (0.0139/(pow((x + 0.3327), 6.3391)));  
    float mSigma = -4.42;
    double Pion_lower = mSigma*f_nSigmaTOFPionTrack_res + f_nSigmaTOFPionTrack_pos;
...
void Danalysis_dstar(TString Filename, TString OutputFilename, Int_t ndiv = 1)
{
    ...
    TList *list;
    if (file) {
        list = dynamic_cast<TList*>(file->Get("picoD0AnaMaker"));
        /*if (list) {
        hEventStat1 = dynamic_cast<TH1F*>(list->FindObject("hEventStat1"));
        }*/
        file->Close();
    }

    TChain *ntp_RightSign = new TChain("Dntp_RightSign");
    ntp_RightSign->Add(Filename);
    TChain *ntp_SideBand = new TChain("Dntp_SideBand");
    ntp_SideBand->Add(Filename);
    TChain *ntp_ME = new TChain("Dntp_signal_ME");
    ntp_ME->Add(Filename);
    ////ntp_ME->Add("outputLocal.picoMEtree.sigME.root");
    TChain *ntp_WrongSign = new TChain("Dntp_Wrongsign");
    ntp_WrongSign->Add(Filename);
    ...
    Float_t primVz, primVzVpd, k_nHitFit, pi1_nHitFit, pi2_nHitFit, k_gDCA, pi1_gDCA, pi2_gDCA, k_pt, pi1_pt, pi2_pt, k_nSigmaTPC, pi1_nSigmaTPC, pi2_nSigmaTPC, k_p, pi1_p, pi2_p, k_isTOFmatched, pi1_isTOFmatched, pi2_isTOFmatched, k_isBEMCmatched, pi1_isBEMCmatched, pi2_isBEMCmatched, k_nSigmaTOF, pi1_nSigmaTOF, pi2_nSigmaTOF, k_charge, pi1_charge, pi2_charge, D_pt, D_cosThetaStar, Dstar_rapidity, Dstar_mass, D_mass, Dstar_pt, pi1_clusterHighestTowerE, pi1_MatchedClusterhighestTowerAdc;
     
    ntp_RightSign -> SetBranchAddress("VzTPC", &primVz);
    ntp_RightSign -> SetBranchAddress("VzVPD", &primVzVpd);

    ntp_RightSign -> SetBranchAddress("k_nHitsFit", &k_nHitFit);
    ntp_RightSign -> SetBranchAddress("pi1_nHitsFit", &pi1_nHitFit);
    ntp_RightSign -> SetBranchAddress("pi2_nHitsFit", &pi2_nHitFit);
    ntp_RightSign -> SetBranchAddress("k_gDCA", &k_gDCA);
    ntp_RightSign -> SetBranchAddress("pi1_gDCA", &pi1_gDCA);
    ntp_RightSign -> SetBranchAddress("pi2_gDCA", &pi2_gDCA);
    ntp_RightSign -> SetBranchAddress("k_pt", &k_pt);
    ntp_RightSign -> SetBranchAddress("pi1_pt", &pi1_pt);
    ntp_RightSign -> SetBranchAddress("pi2_pt", &pi2_pt);
    ...
    ntp_RightSign -> SetBranchAddress("k_isTOFmatched", &k_isTOFmatched);
    ntp_RightSign -> SetBranchAddress("pi1_isTOFmatched", &pi1_isTOFmatched);
    ntp_RightSign -> SetBranchAddress("pi2_isTOFmatched", &pi2_isTOFmatched);
    ntp_RightSign -> SetBranchAddress("k_isBEMCmatched", &k_isBEMCmatched);
    ntp_RightSign -> SetBranchAddress("pi1_isBEMCmatched", &pi1_isBEMCmatched);
    ntp_RightSign -> SetBranchAddress("pi2_isBEMCmatched", &pi2_isBEMCmatched);
    ...
    ntp_RightSign -> SetBranchAddress("D_pt", &D_pt);
    ntp_RightSign -> SetBranchAddress("D_cosThetaStar", &D_cosThetaStar);
    
    ntp_RightSign -> SetBranchAddress("Dstar_rapidity", &Dstar_rapidity);
    ntp_RightSign -> SetBranchAddress("Dstar_mass", &Dstar_mass);
    ntp_RightSign -> SetBranchAddress("D_mass", &D_mass);
    ntp_RightSign -> SetBranchAddress("Dstar_pt", &Dstar_pt);
    /*ntp_RightSign -> SetBranchAddress("pi1_clusterHighestTowerE", &pi1_clusterHighestTowerE);
    ntp_RightSign -> SetBranchAddress("pi1_MatchedClusterhighestTowerAdc", &pi1_MatchedClusterhighestTowerAdc);*/


    Float_t sprimVz, sprimVzVpd, sk_nHitFit, spi1_nHitFit, spi2_nHitFit, sk_gDCA, spi1_gDCA, spi2_gDCA, sk_pt, spi1_pt, spi2_pt, sk_nSigmaTPC, spi1_nSigmaTPC, spi2_nSigmaTPC, sk_p, spi1_p, spi2_p, sk_isTOFmatched, spi1_isTOFmatched, spi2_isTOFmatched, sk_isBEMCmatched, spi1_isBEMCmatched, spi2_isBEMCmatched, sk_nSigmaTOF, spi1_nSigmaTOF, spi2_nSigmaTOF, sk_charge, spi1_charge, spi2_charge, sD_pt, sD_cosThetaStar, sDstar_rapidity, sDstar_mass, sD_mass, sDstar_pt, spi1_clusterHighestTowerE, spi1_MatchedClusterhighestTowerAdc;
    
    ntp_SideBand -> SetBranchAddress("VzTPC", &sprimVz);
    ntp_SideBand -> SetBranchAddress("VzVPD", &sprimVzVpd);

    ntp_SideBand -> SetBranchAddress("k_nHitsFit", &sk_nHitFit);
    ntp_SideBand -> SetBranchAddress("pi1_nHitsFit", &spi1_nHitFit);
    ntp_SideBand -> SetBranchAddress("pi2_nHitsFit", &spi2_nHitFit);
    ntp_SideBand -> SetBranchAddress("k_gDCA", &sk_gDCA);
    ntp_SideBand -> SetBranchAddress("pi1_gDCA", &spi1_gDCA);
    ntp_SideBand -> SetBranchAddress("pi2_gDCA", &spi2_gDCA);
    
    ntp_SideBand -> SetBranchAddress("k_pt", &sk_pt);
    ntp_SideBand -> SetBranchAddress("pi1_pt", &spi1_pt);
    ntp_SideBand -> SetBranchAddress("pi2_pt", &spi2_pt);
    ...
    ntp_SideBand -> SetBranchAddress("k_isTOFmatched", &sk_isTOFmatched);
    ntp_SideBand -> SetBranchAddress("pi1_isTOFmatched", &spi1_isTOFmatched);
    ntp_SideBand -> SetBranchAddress("pi2_isTOFmatched", &spi2_isTOFmatched);
    ntp_SideBand -> SetBranchAddress("k_isBEMCmatched", &sk_isBEMCmatched);
    ntp_SideBand -> SetBranchAddress("pi1_isBEMCmatched", &spi1_isBEMCmatched);
    ntp_SideBand -> SetBranchAddress("pi2_isBEMCmatched", &spi2_isBEMCmatched);
    ...
    ntp_SideBand -> SetBranchAddress("D_pt", &sD_pt);
    ntp_SideBand -> SetBranchAddress("D_cosThetaStar", &sD_cosThetaStar);

    ntp_SideBand -> SetBranchAddress("Dstar_rapidity", &sDstar_rapidity);
    ntp_SideBand -> SetBranchAddress("Dstar_mass", &sDstar_mass);
    ntp_SideBand -> SetBranchAddress("D_mass", &sD_mass);
    ntp_SideBand -> SetBranchAddress("Dstar_pt", &sDstar_pt);
    /*ntp_SideBand -> SetBranchAddress("pi1_clusterHighestTowerE", &spi1_clusterHighestTowerE);
    ntp_SideBand -> SetBranchAddress("pi1_MatchedClusterhighestTowerAdc", &spi1_MatchedClusterhighestTowerAdc);*/

    Float_t wprimVz, wprimVzVpd, wk_nHitFit, wpi1_nHitFit, wpi2_nHitFit, wk_gDCA, wpi1_gDCA, wpi2_gDCA, wk_pt, wpi1_pt, wpi2_pt, wk_nSigmaTPC, wpi1_nSigmaTPC, wpi2_nSigmaTPC, wk_p, wpi1_p, wpi2_p, wk_isTOFmatched, wpi1_isTOFmatched, wpi2_isTOFmatched, wk_isBEMCmatched, wpi1_isBEMCmatched, wpi2_isBEMCmatched, wk_nSigmaTOF, wpi1_nSigmaTOF, wpi2_nSigmaTOF, wk_charge, wpi1_charge, wpi2_charge, wD_pt, wD_cosThetaStar, wDstar_rapidity, wDstar_mass, wD_mass, wDstar_pt, wpi1_clusterHighestTowerE, wpi1_MatchedClusterhighestTowerAdc;

    ntp_WrongSign -> SetBranchAddress("VzTPC", &wprimVz);
    ntp_WrongSign -> SetBranchAddress("VzVPD", &wprimVzVpd);

    ntp_WrongSign -> SetBranchAddress("k_nHitsFit", &wk_nHitFit);
    ntp_WrongSign -> SetBranchAddress("pi1_nHitsFit", &wpi1_nHitFit);
    ntp_WrongSign -> SetBranchAddress("pi2_nHitsFit", &wpi2_nHitFit);
    ntp_WrongSign -> SetBranchAddress("k_gDCA", &wk_gDCA);
    ntp_WrongSign -> SetBranchAddress("pi1_gDCA", &wpi1_gDCA);
    ntp_WrongSign -> SetBranchAddress("pi2_gDCA", &wpi2_gDCA);
    
    ntp_WrongSign -> SetBranchAddress("k_pt", &wk_pt);
    ntp_WrongSign -> SetBranchAddress("pi1_pt", &wpi1_pt);
    ntp_WrongSign -> SetBranchAddress("pi2_pt", &wpi2_pt);

    ntp_WrongSign -> SetBranchAddress("k_nSigmaTPC", &wk_nSigmaTPC);
    ntp_WrongSign -> SetBranchAddress("pi1_nSigmaTPC", &wpi1_nSigmaTPC);
    ntp_WrongSign -> SetBranchAddress("pi2_nSigmaTPC", &wpi2_nSigmaTPC);
    ...
    ntp_WrongSign -> SetBranchAddress("k_isTOFmatched", &wk_isTOFmatched);
    ntp_WrongSign -> SetBranchAddress("pi1_isTOFmatched", &wpi1_isTOFmatched);
    ntp_WrongSign -> SetBranchAddress("pi2_isTOFmatched", &wpi2_isTOFmatched);
    ntp_WrongSign -> SetBranchAddress("k_isBEMCmatched", &wk_isBEMCmatched);
    ntp_WrongSign -> SetBranchAddress("pi1_isBEMCmatched", &wpi1_isBEMCmatched);
    ntp_WrongSign -> SetBranchAddress("pi2_isBEMCmatched", &wpi2_isBEMCmatched);
    ntp_WrongSign -> SetBranchAddress("k_nSigmaTOF", &wk_nSigmaTOF);
    ntp_WrongSign -> SetBranchAddress("pi1_nSigmaTOF", &wpi1_nSigmaTOF);
    ntp_WrongSign -> SetBranchAddress("pi2_nSigmaTOF", &wpi2_nSigmaTOF);
    ...
    ntp_WrongSign -> SetBranchAddress("D_pt", &wD_pt);
    ...
    ntp_WrongSign -> SetBranchAddress("Dstar_mass", &wDstar_mass);
    ntp_WrongSign -> SetBranchAddress("D_mass", &wD_mass);
    ntp_WrongSign -> SetBranchAddress("Dstar_pt", &wDstar_pt);
    /*ntp_WrongSign -> SetBranchAddress("pi1_clusterHighestTowerE", &wpi1_clusterHighestTowerE);
    ntp_WrongSign -> SetBranchAddress("pi1_MatchedClusterhighestTowerAdc", &wpi1_MatchedClusterhighestTowerAdc);*/





    /*Float_t mtriplet_pairdiff, mDstar_rapidity, /*mD0_decayL, mD0_theta, mcosTheta, mDstar_pt, /*mpi1_pt, mk_pt,  mpi1_gDCA, mk_gDCA, mk_nSigmaTPC, mpi1_nSigmaTPC, mk_nSigmaTOF, mpi1_nSigmaTOF, mpi1_TOFinvbeta, mk_TOFinvbeta, mpi1_betaBase, mk_betaBase, mD_cosThetaStar, mdcaD0ToPv, mdcaDaughters, mprimVz, mprimVzVpd, mk_nHitFit, mdcaMax, mpi1_eventId, mk_eventId,  mpi1_nHitFit, mpi1_p, mk_p, mpi1_charge, mk_charge, /*mVzVPD_VzTPC, mD_phi, mD_eta, mpi2_charge;
    ...
    ntp_ME -> SetBranchAddress("pi1_gDCA", &mpi1_gDCA);
    ntp_ME -> SetBranchAddress("k_gDCA", &mk_gDCA);
    ...
    ntp_ME -> SetBranchAddress("k_nHitsFit", &mk_nHitFit);
    ntp_ME -> SetBranchAddress("pi1_nHitsFit", &mpi1_nHitFit);
    ...
    TH2F *DstarPlusRightSign = new TH2F("DstarPlusRightSign","DeltaM of DstarPlus RightSign",500,0,50.,5000,0,1);
    TH2F *DstarMinusRightSign = new TH2F("DstarMinusRightSign","DeltaM of DstarMinus RightSign",500,0,50.,5000,0,1); 
    TH2F *DstarPlusSideBand = new TH2F("DstarPlusSideBand","DeltaM of DstarPlus SideBand",500,0,50.,5000,0,1);
    TH2F *DstarMinusSideBand = new TH2F("DstarMinusSideBand","DeltaM of DstarMinus SideBand",500,0,50.,5000,0,1);
    TH2F *MixedDstarPlusRightSign = new TH2F("MixedDstarPlusRightSign","Mixed event background",500,0,50.,5000,0,1);
    TH2F *MixedDstarMinusRightSign = new TH2F("MixedDstarMinusRightSign","Mixed event background",500,0,50.,5000,0,1);
    TH2F *DstarPlusWrongSign = new TH2F("DstarPlusWrongSign","DeltaM of DstarPlus WrongSign",500,0,50.,5000,0,1); 
    TH2F *DstarMinusWrongSign = new TH2F("DstarMinusWrongSign","DeltaM of DstarMinus WrongSign",500,0,50.,5000,0,1);

    TH2F *CosthetastarpT = new TH2F("CosthetastarpT","cosThetaStar vs pT",1000,0,10.,1000,-1,1);
    TH2F *CosthetastarDstarMass = new TH2F("CosthetastarDstarMass","cosThetaStar vs DstarMass",5000,0,1,1000,-1,1);
    ...
    TH1D *pi_gDCA_unlike = new TH1D("pi_gDCA_unlike","pi_gDCA; DCA #pi [cm] ; Entries",100,-1.0, 5.0);
    TH1D *k_gDCA_unlike = new TH1D("k_gDCA_unlike","k_gDCA;  DCA k [cm]; Entries",100,-1.0, 5.0);
    ...
    TH1D *pi_gDCA_like = new TH1D("pi_gDCA_like","pi_gDCA; DCA #pi [cm] ; Entries",100,-1.0, 5.0);
    TH1D *k_gDCA_like = new TH1D("k_gDCA_like","k_gDCA;  DCA k [cm]; Entries",100,-1.0, 5.0);
    ...
    TH1D *pi_gDCA_rot = new TH1D("pi_gDCA_rot","pi_gDCA; DCA #pi [cm] ; Entries",100,-1.0, 5.0);
    TH1D *k_gDCA_rot = new TH1D("k_gDCA_rot","k_gDCA;  DCA k [cm]; Entries",100,-1.0, 5.0);
    ...
    TH1D *pi_gDCA_mixed = new TH1D("pi_gDCA_mixed","pi_gDCA; DCA #pi [cm] ; Entries",100,-1.0, 5.0);
    TH1D *k_gDCA_mixed = new TH1D("k_gDCA_mixed","k_gDCA;  DCA k [cm]; Entries",100,-1.0, 5.0);
    ...
    for (Long64_t i = 0; i < numberEntr; i++) {
        ////////if (i < (ndiv*numberEntr)/200 || i >= ((ndiv+1)*numberEntr)/200) continue;
        if (i%1000000==0) {cout<<"RightSign Dstar "<<i<<endl;}
        ntp_RightSign -> GetEntry(i);

        if (TMath::Abs(primVz) >= 60.0) continue;
        if (TMath::Abs(primVzVpd - primVz) >= 4.0) continue;        
        
        if (k_nHitFit < 18 || pi1_nHitFit < 18 || pi2_nHitFit < 18) continue;
        if (k_gDCA >= 1.5 || pi1_gDCA >= 1.5 || pi2_gDCA >= 1.5) continue;
        if (k_pt <= 0.20 || pi1_pt <= 0.20 || pi2_pt <= 0.1) continue;
        ...
        if (pi1_isTOFmatched == 0) {
            if (pi1_pt <= 1.6 || TMath::Abs(pi1_nSigmaTPC) >= 2.5) continue;
            }
        if (pi2_isTOFmatched == 1) {
                if (pi2_nSigmaTOF <= nSigmaTOFSoftPionTrack_lower(pi2_p) || pi2_nSigmaTOF >= nSigmaTOFSoftPionTrack_upper(pi2_p)) continue;
                }
        

        if (D_mass <= 1.8 || D_mass >= 1.95) continue;



        CosthetastarpT->Fill(D_pt,D_cosThetaStar);
        CosthetastarDstarMass->Fill((Dstar_mass - D_mass) ,D_cosThetaStar);


        if(TMath::Abs(Dstar_rapidity)>=1.0) continue;
        ////////if (D_cosThetaStar >= 0.8) continue;
        ...
        if(pi1_charge == 1 && k_charge == -1 && pi2_charge == 1) {
            ...
            /////if(((triplet_pairdiff>1.78) && (triplet_pairdiff<1.83))){
               ...
               pi_gDCA_unlike->Fill(pi1_gDCA);
               k_gDCA_unlike->Fill(k_gDCA);
            ...
            //////if((D_cosThetaStar < cosThetaStarCutD0) && (TMath::Abs(cos(D0_theta)) > cosThetapointD0cut)) {
                ///////if((D_cosThetaStar < cosThetaStarCutD0)){
                    DstarPlusRightSign->Fill(Dstar_pt,(Dstar_mass - D_mass));
        ...
        if(pi1_charge == -1 && k_charge == 1 && pi2_charge == -1) {
            ...
            //////if((D_cosThetaStar < cosThetaStarCutD0) && (TMath::Abs(cos(D0_theta)) > cosThetapointD0cut)) {
            ///////if((D_cosThetaStar < cosThetaStarCutD0)){
                    DstarMinusRightSign->Fill(Dstar_pt,(Dstar_mass - D_mass));
                    ////hRapidityD->Fill(Dstar_pt,Dstar_rapidity);
            ///////}               
            /////}
            /*else{
    ...
    for (Long64_t i = 0; i < numberEntr; i++) {
        ////////if (i < (ndiv*numberEntr)/200 || i >= ((ndiv+1)*numberEntr)/200) continue;
        if (i%1000000==0) {cout<< "Sideband Dstar "<<i<<endl;}
        ntp_SideBand -> GetEntry(i);
        if (TMath::Abs(sprimVz) >= 60.0) continue;
        if (TMath::Abs(sprimVzVpd - sprimVz) >= 4.0) continue;

        if (sk_nHitFit < 18 || spi1_nHitFit < 18 || spi2_nHitFit < 18) continue;
        if (sk_gDCA >= 1.5 || spi1_gDCA >= 1.5 || spi2_gDCA >= 1.5) continue;
        if (sk_pt <= 0.20 || spi1_pt <= 0.20 || spi2_pt <= 0.1) continue;
        ...
        if (sk_isTOFmatched == 1) {
            if (sk_nSigmaTOF <= nSigmaTOFKaonTrack_lower(sk_p) || sk_nSigmaTOF >= nSigmaTOFKaonTrack_upper(sk_p)) continue;
            } 
        if (sk_isTOFmatched == 0) {
            continue; 
            }
        if (spi1_isTOFmatched == 1) {
            if (spi1_nSigmaTOF <= nSigmaTOFPionTrack_lower(spi1_p) || spi1_nSigmaTOF >= nSigmaTOFPionTrack_upper(spi1_p)) continue;
            }
        if (spi1_isTOFmatched == 0) {
            if (spi1_pt <= 1.6 || TMath::Abs(spi1_nSigmaTPC) >= 2.5) continue;
        }
        if (spi2_isTOFmatched == 1) {
            if (spi2_nSigmaTOF <= nSigmaTOFSoftPionTrack_lower(spi2_p) || spi2_nSigmaTOF >= nSigmaTOFSoftPionTrack_upper(spi2_p)) continue;
            }
        
            

        if (sD_mass <= 1.64 || sD_mass >= 2.11 || (sD_mass >= 1.74 && sD_mass <= 2.01)) continue;
        ////////if(spi2_nSigmaTOF <= nSigmaTOFPionTrack_lower(spi2_p) || spi2_nSigmaTOF >= nSigmaTOFPionTrack_upper(spi2_p)) continue;

        //////if(sD0_decayL < 1.0) continue;  ///could be strict
        //////if(sdcaD0ToPv > 99.9) continue;  ///could be strict
        ///////if(TMath::Abs(scosTheta) > 0.90) continue; ////come back here
        ///////if(TMath::Abs(sD_eta) > 1.0) continue;
        
        //////if(sD_cosThetaStar >= 0.7) continue;  ////come back here

        if(TMath::Abs(sDstar_rapidity)>=1.0) continue;
        //////if (sD_cosThetaStar >= 0.8) continue;
        ...
        if(spi1_charge == 1 && sk_charge == -1 && spi2_charge == 1) {
            ...
            /////if(((striplet_pairdiff>1.78) && (striplet_pairdiff<1.83))){
               /*hNHitsKvsD0pt->Fill(sDstar_pt,sk_nHitFit);
                hNHitsPivsD0pt->Fill(sDstar_pt,spi1_nHitFit);
                hDptPiPt->Fill(sDstar_pt,spi1_pt);
                hDptKPt->Fill(sDstar_pt,sk_pt);
                hDptPiP->Fill(sDstar_pt,spi1_p);
                hDptKP->Fill(sDstar_pt,sk_p);
                hPtPionVsKaon->Fill(spi1_pt,sk_pt);*/
                /*dcaDaughters_unlike->Fill(sdcaDaughters);
                D_theta_unlike->Fill(sD0_theta);
                cosTheta_unlike->Fill(scosTheta);
                D_decayL_unlike->Fill(sD0_decayL);
                dcaD0ToPv_unlike->Fill(sdcaD0ToPv);
                D_cosThetaStar_unlike->Fill(sD_cosThetaStar);
                Dstar_pt_unlike->Fill(sDstar_pt);
                triplet_pairdiff_unlike->Fill(striplet_pairdiff);
                Dstar_rapidity_unlike->Fill(sDstar_rapidity);
                D_phi_unlike->Fill(sD_phi);
                D_eta_unlike->Fill(sD_eta);
                pi_gDCA_unlike->Fill(spi1_gDCA);
                k_gDCA_unlike->Fill(sk_gDCA);
                D_y_D_eta_unlike->Fill(sDstar_rapidity, sD_eta);*/
            /////}
            /////if(sDstar_pt>pTforCos) {
            /////if((sD_cosThetaStar < cosThetaStarCutD0) && (TMath::Abs(cos(sD0_theta)) > cosThetapointD0cut)) {
            /////if((sD_cosThetaStar < cosThetaStarCutD0)){
                    DstarPlusSideBand->Fill(sDstar_pt,(sDstar_mass - sD_mass));
                    ////hRapidityD->Fill(sDstar_pt,sDstar_rapidity);
            /////}
            /////}
            /*else{
        ...
        if(spi1_charge == -1 && sk_charge == 1 && spi2_charge == -1) {
            ...
            /////if((sD_cosThetaStar < cosThetaStarCutD0) && (TMath::Abs(cos(sD0_theta)) > cosThetapointD0cut)) {
            /////if((sD_cosThetaStar < cosThetaStarCutD0)){
                    DstarMinusSideBand->Fill(sDstar_pt,(sDstar_mass - sD_mass));
                    ////hRapidityD->Fill(sDstar_pt,sDstar_rapidity);
            /////}
            /////}
            /*else{
    ...
    for (Long64_t i = 0; i < numberEntr; i++) {
        ////////if (i < (ndiv*numberEntr)/200 || i >= ((ndiv+1)*numberEntr)/200) continue;
        if (i%1000000==0) {cout<< "WrongSign Dstar "<<i<<endl;}
        ntp_WrongSign -> GetEntry(i);
        if (TMath::Abs(wprimVz) >= 60.0) continue;
        if (TMath::Abs(wprimVzVpd - wprimVz) >= 4.0) continue;

        
        if (wk_nHitFit < 18 || wpi1_nHitFit < 18 || wpi2_nHitFit < 18) continue;
        if (wk_gDCA >= 1.5 || wpi1_gDCA >= 1.5 || wpi2_gDCA >= 1.5) continue;
        if (wk_pt <= 0.20 || wpi1_pt <= 0.2 || wpi2_pt <= 0.1) continue;
        ...
        if (wk_isTOFmatched == 1) {
            if (wk_nSigmaTOF <= nSigmaTOFKaonTrack_lower(wk_p) || wk_nSigmaTOF >= nSigmaTOFKaonTrack_upper(wk_p)) continue;
            } 
        if (wk_isTOFmatched == 0) {
            continue; 
            }
        if (wpi1_isTOFmatched == 1) {
            if (wpi1_nSigmaTOF <= nSigmaTOFPionTrack_lower(wpi1_p) || wpi1_nSigmaTOF >= nSigmaTOFPionTrack_upper(wpi1_p)) continue;
            }
        if (wpi1_isTOFmatched == 0) {
            if (wpi1_pt <= 1.6 || TMath::Abs(wpi1_nSigmaTPC) >= 2.5) continue;
            }
       if (wpi2_isTOFmatched == 1) {
            if (wpi2_nSigmaTOF <= nSigmaTOFSoftPionTrack_lower(wpi2_p) || wpi2_nSigmaTOF >= nSigmaTOFSoftPionTrack_upper(wpi2_p)) continue;
            }
        
        
        if (wD_mass <= 1.8 || wD_mass >= 1.95) continue;  
            //////if(wD0_decayL < 1.0) continue;  ///could be strict
        ...
        ///////if (wD_cosThetaStar >= 0.8) continue;
        ...
        if(wpi1_charge == 1 && wk_charge == -1 && wpi2_charge == -1) {
            ...
            /////if(((wtriplet_pairdiff>1.78) && (wtriplet_pairdiff<1.83))){
               /*hNHitsKvsD0pt->Fill(wDstar_pt,wk_nHitFit);
                hNHitsPivsD0pt->Fill(wDstar_pt,wpi1_nHitFit);
                hDptPiPt->Fill(wDstar_pt,wpi1_pt);
                hDptKPt->Fill(wDstar_pt,wk_pt);
                hDptPiP->Fill(wDstar_pt,wpi1_p);
                hDptKP->Fill(wDstar_pt,wk_p);
                hPtPionVsKaon->Fill(wpi1_pt,wk_pt);*/
                /*dcaDaughters_unlike->Fill(wdcaDaughters);
                D_theta_unlike->Fill(wD0_theta);
                cosTheta_unlike->Fill(wcosTheta);
                D_decayL_unlike->Fill(wD0_decayL);
                dcaD0ToPv_unlike->Fill(wdcaD0ToPv);
                D_cosThetaStar_unlike->Fill(wD_cosThetaStar);
                Dstar_pt_unlike->Fill(wDstar_pt);
                triplet_pairdiff_unlike->Fill(wtriplet_pairdiff);
                Dstar_rapidity_unlike->Fill(wDstar_rapidity);
                D_phi_unlike->Fill(wD_phi);
                D_eta_unlike->Fill(wD_eta);
                pi_gDCA_unlike->Fill(wpi1_gDCA);
                k_gDCA_unlike->Fill(wk_gDCA);
                D_y_D_eta_unlike->Fill(wDstar_rapidity, wD_eta);*/
            /////}
            ...
            /////if((wD_cosThetaStar < cosThetaStarCutD0)){
                    DstarPlusWrongSign->Fill(wDstar_pt,(wDstar_mass - wD_mass));
                    ////hRapidityD->Fill(wDstar_pt,wDstar_rapidity);
            }
        ...
        if(wpi1_charge == -1 && wk_charge == 1 && wpi2_charge == 1) {
            ...
            /////if((wD_cosThetaStar < cosThetaStarCutD0) && (TMath::Abs(cos(wD0_theta)) > cosThetapointD0cut)) {
            /////if((wD_cosThetaStar < cosThetaStarCutD0)){
                    DstarMinusWrongSign->Fill(wDstar_pt,(wDstar_mass - wD_mass));
                    ////hRapidityD->Fill(wDstar_pt,wDstar_rapidity);
            }
    ...
    for (Long64_t i = 0; i < numberEntr; i++) {
        ...
        ////if(spi1_gDCA > 0.62) continue;
        ////if(sk_gDCA < 0.38) continue;
        if(mpi1_charge == 1 && mk_charge == -1 && mpi2_charge == 1) {
            ...
            /////if(((mtriplet_pairdiff>1.78) && (mtriplet_pairdiff<1.83))){
               ...
               pi_gDCA_mixed->Fill(mpi1_gDCA);
               k_gDCA_mixed->Fill(mk_gDCA);
    ...
    TFile* dataRes = new TFile("output/"+OutputFilename+".root","RECREATE");

    CosthetastarpT->Write();
    CosthetastarDstarMass->Write();
    list->Write();
