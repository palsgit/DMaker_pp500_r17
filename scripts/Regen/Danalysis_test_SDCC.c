...
void Danalysis_test_SDCC(TString Filename, TString OutputFilename)
{
    ...
    for (Long64_t i = 0; i < numberEntr; i++) {
        ...
        if (TMath::Abs(primVz) >= 30.0) continue;
        if (TMath::Abs(primVzVpd - primVz) >= 3.0) continue;
        
        //////////if (k_nHitFit < 15 || pi1_nHitFit < 15) continue;
        ////////if (k_dca >= 3.0 || pi1_dca >= 3.0) continue;
        if (k_pt <= 0.16 || pi1_pt <= 0.16) continue;
        ...
        if (k_isTOFmatched == 0) continue;
        /*  } 
        if (k_pt >= 1.6) {
            ///////if (TMath::Abs(k_nSigmaTPC) >= 2 && k_pt > 1.6) continue;
            if (k_isBEMCmatched == 0 || k_isTOFmatched == 0) continue; 
            }*/
        ...
        if (pi1_isTOFmatched == 0) continue;
        /*   }
        if (pi1_pt >= 1.6) {
            //////if (TMath::Abs(pi1_nSigmaTPC) >= 2 && pi1_pt > 1.6) continue;
        ...
        if (pi1_isTOFmatched == 0) {
            /////if (TMath::Abs(pi1_nSigmaTPC) >= 2 && pi1_pt > 1.6) continue;
            /////if (pi1_isBEMCmatched == 0 || TMath::Abs(pi1_nSigmaTPC) >= 1) continue;
            continue;
            }*/
    ...
    for (Long64_t i = 0; i < numberEntr; i++) {
        ...
        if (TMath::Abs(rprimVz) >= 30.0) continue;
        if (TMath::Abs(rprimVzVpd - rprimVz) >= 3.0) continue;

        //////////if (rk_nHitFit < 15 || rpi1_nHitFit < 15) continue;
        /////////if (rk_dca >= 3.0 || rpi1_dca >= 3.0) continue;
        if (rk_pt <= 0.16 || rpi1_pt <= 0.16) continue;
        ...
        if(rk_nSigmaTPC <= -2.50 || rk_nSigmaTPC >= 3.0) continue; 
        if (rpi1_nSigmaTPC <= -3.0 || rpi1_nSigmaTPC >= 3.0) continue;
        /*if(rk_nSigmaTPC <= -2.5 || rk_nSigmaTPC >= 2.5) continue;
        ...
        if (rk_isTOFmatched == 0) {
            //////if (TMath::Abs(rk_nSigmaTPC) >= 2 && rk_pt > 1.6) continue;
            //////if (rk_isBEMCmatched == 0 || TMath::Abs(rk_nSigmaTPC) >= 1) continue;
            continue;
            }
        ...
        if (rpi1_isTOFmatched == 0) {
            //////if (TMath::Abs(rpi1_nSigmaTPC) >= 2 && rpi1_pt > 1.6) continue;
            //////if(rpi1_isBEMCmatched == 0 || TMath::Abs(rpi1_nSigmaTPC) >= 1) continue;
            continue;
            }
    ...
    for (Long64_t i = 0; i < numberEntr; i++) {
        ...
        if (TMath::Abs(lprimVz) >= 30.0) continue;
        if (TMath::Abs(lprimVzVpd - lprimVz) >= 3.0) continue;

        //////////if (lk_nHitFit < 15 || lpi1_nHitFit < 15) continue;
        /////////if (lk_dca >= 3.0 || lpi1_dca >= 3.0) continue;
        if (lk_pt <= 0.16 || lpi1_pt <= 0.16) continue;
        ...
        if(lk_nSigmaTPC <= -2.50 || lk_nSigmaTPC >= 3.0) continue;
        if (lpi1_nSigmaTPC <= -3.0 || lpi1_nSigmaTPC >= 3.0) continue;
        /*if(lk_nSigmaTPC <= -2.5 || lk_nSigmaTPC >= 2.5) continue;
        ...
        if (lk_isTOFmatched == 0) {
            //////if (TMath::Abs(lk_nSigmaTPC) >= 2 && lk_pt > 1.6) continue;
            ///if (lk_isBEMCmatched == 0 || TMath::Abs(lk_nSigmaTPC) >= 1) continue;
            continue;
            }
        ...
        if (lpi1_isTOFmatched == 0) {
            /////if (TMath::Abs(lpi1_nSigmaTPC) >= 2 && lpi1_pt > 1.6) continue;
            /////if (lpi1_isBEMCmatched == 0 || TMath::Abs(lpi1_nSigmaTPC) >= 1) continue;
            continue;
            }
