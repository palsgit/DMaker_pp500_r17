...
//	gStyle->SetOptFit(0);
	...
	ptBin1 = D->GetXaxis()->FindBin(pT1);
	ptBin2 = D->GetXaxis()->FindBin(pT2);
  ...
  TH1D *MixedDbar_py = (TH1D*)gDirectory->Get("MixedDbar_py");
	...
	TLegend *leg = new TLegend(0.4147564,0.6452174,0.6941261,0.8243478,NULL,"brNDC");
   ...
   leg->SetFillStyle(1001);
	...
	normEdge3 = Unlike->FindBin(1.95);
	normEdge4 = Unlike->FindBin(2.05);
	...
	Scalefactor = (1.0*Scalefactorleft + 0*Scalefactorright);
	////if (pT2 > 2.0) Scalefactor = (0.0*Scalefactorleft + 1.0*Scalefactorright);
	...
	////Scalefactor = (0*Scalefactorleft + 1.0*Scalefactorright);
	Scalefactor = (0.0*Scalefactorleft + 1.0*Scalefactorright);
	if (pT1 > 1.0) Scalefactor = (1.0*Scalefactorleft + 0.0*Scalefactorright);


	///////Scalefactor = 0.9999;
 	//////if (pT2 > 1.5) RotateBg->Scale(Scalefactorleft);
	/*else*/ //RotateBg->Scale(Scalefactorright);
  ...
  cout<<"rotated momentum scale factor: "<<Scalefactor<<endl;
	...
	Unlike->GetYaxis()->SetTitle(Form("Entries (/%.3f GeV/c^{2})", 0.005*rebin));
	Unlike->SetMarkerStyle(4);
	Unlike->GetYaxis()->SetTitleFont(42);
	Unlike->GetYaxis()->SetLabelFont(42);
	Unlike->GetYaxis()->SetRangeUser(-0.5e6, 3.0e6);
	Unlike->Draw("");
	...
	leg->AddEntry(RotateBg,"track rotation","l");
...
//	}
	...
	par[4]=40; par[5]=1.864; par[6]=0.021;
	fSignalLike->SetParameters(par);
	//////fSignalLike->SetParLimits(4,0,1000);
	fSignalLike->SetParLimits(5,1.850,1.890);
	fSignalLike->SetParLimits(6,0.019,0.0285);///0.022
	if (pT2 > 2.0)  fSignalLike->SetParLimits(6,0.023,0.0285); //0.023, 0.024
	if ((pT2 - pT1) > 1.5)  fSignalLike->SetParLimits(6,0.002,0.0285); //0.022, 0.023
	fSignalLike->SetNpx(1000);
//	fSignalLike->FixParameter(4,0);	
//	fSignalLike->FixParameter(5,1.864);
//	fSignalLike->FixParameter(6,0.02);
...
// fSignalLike->FixParameter(1,par[1]);
	...
	c4->cd();
	TPad *infoLike = new TPad("infoLike","info",0.3982412,0.4346667,0.7977387,0.8346667);
	infoLike->SetFillStyle(4000); // Set fill style to transparent
...
//    x2 = 1.92;
	...
	/////fOutskirt->FixParameter(0,2500);
	/////fOutskirt->FixParameter(1,0.0);
...
//	}
	...
	par[4]=40; par[5]=1.864; par[6]=0.028;
	...
	fSignalRotate->SetParLimits(5,1.850,1.890);
	////fSignalRotate->SetParLimits(6,0.023,0.0285);
	fSignalRotate->SetParLimits(6,0.022,0.0285); /// 0.022, 
	if (pT1 < 1.0)  fSignalRotate->SetParLimits(6,0.022,0.0285);
	if (pT1 > 1.0 && pT2 < 1.7)  fSignalRotate->SetParLimits(6,0.023,0.0285); 
...
//	fSignalRotate->FixParameter(1,par[1]);
	...
	sprintf(parstring,"RwYld = %.0f #pm %.0f",signal,signalerror);
	c3->cd();
	TPad *infoRotate = new TPad("infoRotate","info",0.3982412,0.4346667,0.7977387,0.8346667);
	infoRotate->SetFillStyle(4000); // Set fill style to transparent
	...
	tl.DrawLatex(0.05,0.375,parstring);
	...
	par[4]=40; par[5]=1.864; par[6]=0.02;
	...
	fSignalMixed->SetParLimits(5,1.850,1.890);
	fSignalMixed->SetParLimits(6,0.0175,0.0275);
	fSignalMixed->SetNpx(1000);
//	fSignalMixed->FixParameter(4,0);
//	fSignalMixed->FixParameter(5,1.865);
//	fSignalMixed->FixParameter(6,0.021);
	...
	sprintf(parstring,"RwYld = %.0f #pm %.0f",signal,signalerror);
	c2->cd();
	TPad *infoMixed = new TPad("infoMixed","info",0.3969849,0.516,0.798995,0.812);
    ...
    cout<<"*****************************************************************************"<<endl;
	cout << "S/#sqrt{S+B} from mixed-event  =    " <<  sg << endl;
	////sprintf(parstring,"sg = %.3f",sg);
	////tl.DrawLatex(0.05,0.1,parstring);
...
//	resultsLegend->Draw();
	...
	/*likeClon->GetXaxis()->SetRangeUser(1.7, 2.1);
	//////likeClon->GetYaxis()->SetRangeUser(-1200,3500);
	likeClon->GetYaxis()->SetRangeUser(-3100,8100);*/
	likeClon->GetXaxis()->SetRangeUser(1.7, 2.05);
	likeClon->GetYaxis()->SetRangeUser(-2500, 11000);
	if (pT1 > 1.0 and pT2 < 1.7) {
		likeClon->GetYaxis()->SetRangeUser(-2500, 8100);
	}
	if (pT1 > 1.5 and pT2 < 2.2) {
		likeClon->GetYaxis()->SetRangeUser(-3500, 7100);
	}
	likeClon->GetYaxis()->SetTitle(Form("Entries (/%.3f GeV/c^{2})", 0.005*rebin));
	...
	legLike->AddEntry(likeClon, "Data", "lep");
	legLike->AddEntry(fSignalLike, "Fit", "l");
	legLike->Draw();
	...
	/*rotClon->GetXaxis()->SetRangeUser(1.7, 2.1);
	//////rotClon->GetYaxis()->SetRangeUser(-1200,3500);
	rotClon->GetYaxis()->SetRangeUser(-3100,8100);*/
	rotClon->GetXaxis()->SetRangeUser(1.7, 2.05);
	rotClon->GetYaxis()->SetRangeUser(-2500, 11000);
	if (pT1 > 1.0 and pT2 < 1.7) {
		rotClon->GetYaxis()->SetRangeUser(-2500, 8100);
	}
	if (pT1 > 1.5 and pT2 < 2.2) {
		rotClon->GetYaxis()->SetRangeUser(-3500, 7100);
	}
	rotClon->GetYaxis()->SetTitle(Form("Entries (/%.3f GeV/c^{2})", 0.005*rebin));
	...
	legRot->AddEntry(rotClon, "Data", "lep");
	legRot->AddEntry(fSignalRotate, "Fit", "l");
	legRot->Draw();
...
//	gPad->SetLeftMargin(0.15);
	...
	mixClon->GetXaxis()->SetRangeUser(1.7, 2.1);
	mixClon->GetYaxis()->SetRangeUser(-750,2500);
	mixClon->GetYaxis()->SetTitle(Form("Entries (/%.3f GeV/c^{2})", 0.005*rebin));
...
//	D0_Like->Fit("signalFcn","","",0.8,0.95);
	...
	TGaxis* RawYieldAxis = new TGaxis(2.5,0,2.5,1.5e6,0,0.75e6,510,"+L");
	...
	leg->AddEntry(CloneR,"2*(unlike sign - track rotation)","p");
	///leg->AddEntry(CloneR, Form("p_{T} #in [%.1f,%.1f] (GeV/c)",pT1,pT2),"");
