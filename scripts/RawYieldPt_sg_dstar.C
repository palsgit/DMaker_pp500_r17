...
Double_t fitFunction(Double_t *x, Double_t *par) {
	...
	////return parabola(x,par) + normalDistribution(x,par);
}
...
void RawYieldPt_sg_dstar(float pT1, float pT2, int rebin) {
	...
	float x1,x2,lowEdge,highEdge,normEdge1,normEdge2,ptBin1,ptBin2;
	int nBinsInvMass;
...
//	gStyle->SetOptFit(0);
	...
	ptBin1 = DstarPlusRightSign->FindBin(pT1);
	ptBin2 = DstarPlusRightSign->FindBin(pT2);
  ...
  TH1D *DstarMinusSideBand_py = (TH1D*)gDirectory->Get("DstarMinusSideBand_py");
   ...
   nBinsInvMass = DstarPlusRightSign_py->GetNbinsX();
	...
	nBinsInvMass = DstarMinusRightSign_py->GetNbinsX();
	lowEdge = DstarMinusRightSign_py->GetBinLowEdge(1);
	highEdge = DstarMinusRightSign_py->GetBinLowEdge(nBinsInvMass+1);
	...
	DstarPlusRightSign_py->Add(DstarMinusRightSign_py);
	RightSign = DstarPlusRightSign_py;
	MixedDstarPlusRightSign_py->Add(MixedDstarMinusRightSign_py);
	MixedBg = MixedDstarPlusRightSign_py;
	if(GeometricMean){
		float a,b,da,db;
		for(int i=0; i<=nBinsInvMass; i++){
			...
			if(a==0 || b==0) 
			{
				...
				WrongSignBg->SetBinContent(i,2*TMath::Sqrt(a*b));
				WrongSignBg->SetBinError(i,TMath::Sqrt((((da*da*b)/a)+((db*db*a)/b))));
			}
		}
	}
	if(!GeometricMean){
	DstarPlusWrongSign_py->Add(DstarMinusWrongSign_py);
	WrongSignBg = DstarPlusWrongSign_py;}	
	DstarPlusSideBand_py->Add(DstarMinusSideBand_py);
	SideBandBg = DstarPlusSideBand_py;
	Double_t Scalefactor;
	normEdge1 = RightSign->FindBin(0.138);
	normEdge2 = RightSign->FindBin(0.142);
	Scalefactor = RightSign->Integral(normEdge1,normEdge2)/MixedBg->Integral(normEdge1,normEdge2);
	MixedBg->Scale(Scalefactor);
	cout<<"Mixed event Scale factor: "<<Scalefactor<<endl;
	Scalefactor = RightSign->Integral(normEdge1,normEdge2)/SideBandBg->Integral(normEdge1,normEdge2); 
	SideBandBg->Scale(Scalefactor);
  cout<<"SideBand scale factor: "<<Scalefactor<<endl;
	Scalefactor = RightSign->Integral(normEdge1,normEdge2)/WrongSignBg->Integral(normEdge1,normEdge2);
	WrongSignBg->Scale(Scalefactor);
  cout<<"WrongSign scale factor: "<<Scalefactor<<endl;
	...
	par[4]=2.81337e-02; par[5]=1.45534e-01; par[6]=3.00000e-04;
	...
	fSignalMixed->SetParLimits(6,0.0003,0.0007);
...
//	fSignalMixed->FixParameter(6,0.0111);
	...
	float x1a = (fSignalMixed->GetParameter(5) - (3*fSignalMixed->GetParameter(6)));
	float x2a = (fSignalMixed->GetParameter(5) + (3*fSignalMixed->GetParameter(6)));
	...
	float signalgauss = (fsignalonlymix->Integral(x1aBinLowEdge,x2aBinHighEdge))/mixClon->GetBinWidth(1);
...
//    x2 = 1.92;
	...
	for(int i=0; i<= Dstar_SideBand->GetNbinsX(); i++){
	...
	int lowborder = SideBandClon->FindBin(1.83);
	int highborder = SideBandClon->FindBin(1.9);
...
//	fSignalSideBand->FixParameter(0,par[0]);
	...
	fsignalonlySideBand->SetLineWidth(4);
	...
	fresbgonlySideBand->SetLineWidth(4);
...
//    x2=1.92;
  ...
  TH1D *D0_WrongSign;
	...
	D0_WrongSign->Sumw2();
	for(int i=0; i<= D0_WrongSign->GetNbinsX(); i++){
...
//	D0_WrongSign->Fit("fLinearOutskirt","","",x1,x2);
...
//	}
	...
	PlotLine(0.140,0.140,WrongSignClon->GetMinimum(),WrongSignClon->GetMaximum(),1,3,2);
	PlotLine(0.148,0.148,WrongSignClon->GetMinimum(),WrongSignClon->GetMaximum(),1,3,2);
	
	x1Bin = D0_WrongSign->FindBin(x1);
	x2Bin = D0_WrongSign->FindBin(x2);
	nBins = x2Bin - x1Bin;
	...
	fSignalWrongSign->SetParLimits(6,0.0003,0.0007);
	////fSignalWrongSign->SetNpx(1000);
//	fSignalWrongSign->FixParameter(4,0);	
//	fSignalWrongSign->FixParameter(5,1.864);
//	fSignalWrongSign->FixParameter(6,0.011);
...
//	fSignalWrongSign->FixParameter(1,par[1]);
	...
	fSignalWrongSign->SetLineColor(4);
	...
	fsignalonlyWrongSign->SetLineWidth(4);
	...
	fresbgonlyWrongSign->SetLineWidth(4);
	fresbgonlyWrongSign->SetLineStyle(6);
	fresbgonlyWrongSign->Draw("same");
	
	signal = fSignalWrongSign->GetParameter(4)/WrongSignClon->GetBinWidth(1);
	signalerror = fSignalWrongSign->GetParError(4)/WrongSignClon->GetBinWidth(1);
	sprintf(parstring,"RwYld = %.0f #pm %.0f",signal,signalerror);
	c4->cd();
	...
	x1a = (fSignalWrongSign->GetParameter(5) - (3*fSignalWrongSign->GetParameter(6)));
	x2a = (fSignalWrongSign->GetParameter(5) + (3*fSignalWrongSign->GetParameter(6)));
	...
	signalgauss = fsignalonlyWrongSign->Integral(x1aBinLowEdge,x2aBinHighEdge)/WrongSignClon->GetBinWidth(1);
	cout << signalgauss << "     " << sqrt(RightSign->Integral(x1aBin, x2aBin)) << endl;
...
//	hSignal_Mixed->GetXaxis()->SetRangeUser(0.138,0.154);
...
//	gPad->SetLeftMargin(0.15);
	...
	SideBandClon->GetXaxis()->SetRangeUser(0.138,0.154);
	SideBandClon->GetYaxis()->SetRangeUser(-40, 80);
	SideBandClon->GetYaxis()->SetTitle("Entries (/0.0005 GeV/c^{2})");
	...
	WrongSignClon->GetXaxis()->SetRangeUser(0.138,0.154);
	WrongSignClon->GetYaxis()->SetRangeUser(-40, 80);
	WrongSignClon->GetYaxis()->SetTitle("Entries (/0.0005 GeV/c^{2})");
...
//	gPad->SetLeftMargin(0.15);
	...
	mixClon->GetXaxis()->SetRangeUser(0.138,0.154);
	mixClon->GetYaxis()->SetRangeUser(-40, 80);
	mixClon->GetYaxis()->SetTitle("Entries (/0.0005 GeV/c^{2})");
