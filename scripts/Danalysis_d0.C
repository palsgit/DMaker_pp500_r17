...
void Danalysis_d0(TString Filename, TString OutputFilename, Int_t ndiv = 1)
{
    ...
    ntp_signal -> SetBranchAddress("pi1_gDCA", &pi1_gDCA);
    ntp_signal -> SetBranchAddress("k_gDCA", &k_gDCA);
    ...
    ntp_signal -> SetBranchAddress("runId", &runID);
    
    
    Float_t rrunID;
    Float_t rD0_mass, rD0_rapidity, rD0_decayL, rD0_theta, rcosTheta,  rD0_pt, rpi1_pt, rk_pt,  rpi1_gDCA, rk_gDCA, rk_nSigmaTPC, rpi1_nSigmaTPC, rk_nSigmaTOF, rpi1_nSigmaTOF, rpi1_TOFinvbeta, rk_TOFinvbeta, rpi1_betaBase, rk_betaBase, rD_cosThetaStar, rdcaD0ToPv, rdcaDaughters, rprimVz, rprimVzVpd, rk_nHitFit, rdcaMax, rpi1_eventId, rk_eventId,  rpi1_nHitFit, rpi1_p, rk_p, rpi1_charge, rk_charge, rVzVPD_VzTPC, rD_phi, rD_eta, rk_isTOFmatched, rpi1_isTOFmatched, rpi1_NHitsDedx, rk_NHitsDedx, rpi1_btofYLocal, rk_btofYLocal, rk_isBEMCmatched, rpi1_isBEMCmatched;
    ...
    ntp_Rotated -> SetBranchAddress("pi1_gDCA", &rpi1_gDCA);
    ntp_Rotated -> SetBranchAddress("k_gDCA", &rk_gDCA);
    ...
    Float_t lrunID;
    Float_t lD0_mass, lD0_rapidity, lD0_decayL, lD0_theta, lcosTheta, lD0_pt, lpi1_pt, lk_pt,  lpi1_gDCA, lk_gDCA, lk_nSigmaTPC, lpi1_nSigmaTPC, lk_nSigmaTOF, lpi1_nSigmaTOF, lpi1_TOFinvbeta, lk_TOFinvbeta, lpi1_betaBase, lk_betaBase, lD_cosThetaStar, ldcaD0ToPv, ldcaDaughters, lprimVz, lprimVzVpd, lk_nHitFit, ldcaMax, lpi1_eventId, lk_eventId,  lpi1_nHitFit, lpi1_p, lk_p, lpi1_charge, lk_charge, lVzVPD_VzTPC, lD_phi, lD_eta, lk_isTOFmatched, lpi1_isTOFmatched, lpi1_NHitsDedx, lk_NHitsDedx, lpi1_btofYLocal, lk_btofYLocal, lk_isBEMCmatched, lpi1_isBEMCmatched;
    ...
    ntp_LikeSign -> SetBranchAddress("pi1_gDCA", &lpi1_gDCA);
    ntp_LikeSign -> SetBranchAddress("k_gDCA", &lk_gDCA);
    ...
    ntp_LikeSign -> SetBranchAddress("runId", &lrunID);
    ...
    ifstream BadRunList("/gpfs/mnt/gpfs02/eic/palspeic/STAR/MINBIAS/2024-09-20_06-06_D0DStar_MinBias_AlmostEverythingIncluded/bad_run_list3sigma.txt");
    std::vector<float> RunNumberVector;
    if (BadRunList.is_open()) {
        int Run;
        while (BadRunList >> Run) {
            RunNumberVector.push_back(Run);
        }
    } else {
        cout<<"Failed to open BadRunList!"<<endl;
    }
    ...
    for (Long64_t i = 0; i < numberEntr; i++) {
        //////if (i < (ndiv*numberEntr)/600 || i >= ((ndiv+1)*numberEntr)/600) continue;
    ...
    /*if (std::find(RunNumberVector.begin(), RunNumberVector.end(), runID) != RunNumberVector.end()) {
        continue;
    }*/
        ...
        if (k_gDCA >= 1.5 || pi1_gDCA >= 1.5) continue;
        ...
        */
        ...
        /////if(pi1_gDCA > 0.62) continue;  ///????
        /////if(k_gDCA < 0.38) continue;*/
    ...
    for (Long64_t i = 0; i < numberEntr; i++) {
        ...
        /*if (std::find(RunNumberVector.begin(), RunNumberVector.end(), rrunID) != RunNumberVector.end()) {
            continue;
        }*/
        ...
        if (rk_gDCA >= 1.5 || rpi1_gDCA >= 1.5) continue;
        ...
        ////if(rpi1_gDCA > 0.62) continue;
        ////if(rk_gDCA < 0.38) continue;
        ...
        if(rpi1_charge == 1 && rk_charge == -1) {
            ...
            /////if(((rD0_mass>1.78) && (rD0_mass<1.83))){
              /* dcaDaughters_rot->Fill(rdcaDaughters);
               ...
               pi_gDCA_rot->Fill(rpi1_gDCA);
               k_gDCA_rot->Fill(rk_gDCA);
    ...
    for (Long64_t i = 0; i < numberEntr; i++) {
        ...
        if (mk_pt <= 0.2 || mpi1_pt <= 0.2) continue;

        if(mk_nSigmaTPC <= -2.5 || mk_nSigmaTPC >= 3.0) continue; 
        if (mpi1_nSigmaTPC <= -3.0 || mpi1_nSigmaTPC >= 3.0) continue;

        if (mk_isTOFmatched == 1) {
            if (mk_nSigmaTOF <= nSigmaTOFKaonTrack_lower(mk_p) || mk_nSigmaTOF >= nSigmaTOFKaonTrack_upper(mk_p)) continue;
            } 
        if (mk_isTOFmatched == 0) {
            //////if (TMath::Abs(rk_nSigmaTPC) >= 1.0) continue;
            /////if (rk_isBEMCmatched == 0 || TMath::Abs(rk_nSigmaTPC) >= 1.0) continue;
            continue; 
            }

        if (mpi1_isTOFmatched == 1) {
            if (mpi1_nSigmaTOF <= nSigmaTOFPionTrack_lower(mpi1_p) || mpi1_nSigmaTOF >= nSigmaTOFPionTrack_upper(mpi1_p)) continue;
            //////if (rpi1_nSigmaTOF <= -2.5 || rpi1_nSigmaTOF >= 2.5) continue;
            }
        if (mpi1_isTOFmatched == 0) {
            if (TMath::Abs(mpi1_nSigmaTPC) >= 2.5) continue;
            /////continue;
            }
    ...
    for (Long64_t i = 0; i < numberEntr; i++) {
        ...
        /*if (std::find(RunNumberVector.begin(), RunNumberVector.end(), lrunID) != RunNumberVector.end()) {
            continue;
        }*/
        ...
        if (lk_gDCA >= 1.5 || lpi1_gDCA >= 1.5) continue;
        ...
        if(lpi1_charge == 1 && lk_charge == 1 ) {
            ...
            /////if(((lD0_mass>1.78) && (lD0_mass<1.83))){
              /* dcaDaughters_like->Fill(ldcaDaughters);
               ...
               pi_gDCA_like->Fill(lpi1_gDCA);
               k_gDCA_like->Fill(lk_gDCA);
